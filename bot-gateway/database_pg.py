...
def _cleanup_seeking_listings():
    ...
    seeking_pattern = (
        r'ищ[уёемаю]\s+(квартир|жиль|комнат|студи|[0-9])|'
        r'ищем\s+(квартир|комнат|жиль|аренд|[0-9])|'
        r'ищем.*комнатн|'
        ...
        r'семья?\s+ищет|пара\s+ищет|молодая\s+пара|'
        r'срочно\s+ищ[уем]|'
        r'мне\s+нужна\s+квартир|нам\s+нужна\s+(квартир|комнат)|'
        r'мы\s+ищем\s+(квартир|комнат|[0-9])|'
        r'сниму\s+(квартир|комнат)|снимем\s+квартир|'
        r'ищу\s+соседк|ищу\s+соседа|ищу\s+сожител|'
        r'looking\s+for\s+([0-9]+\s+)?(an?\s+)?(apart|flat|room)|'
        r'searching\s+for\s+([0-9]+\s+)?(an?\s+)?(apart|flat|room)|'
        r'want\s+to\s+rent|'
        r'need\s+(an?\s+)?(apart|flat|room|place\s+to)|'
        r'מחפש[תים]?\s+(דירה|דירת|חדר|שכירות)|'
        r'מחפשים\s+(דירה|דירת|חדר|להשכיר|שותף|שותפ)|'
        r'מחפשת\s+(דירה|דירת|חדר|שותפ)|'
        r'בחיפוש\s+(אחר\s+)?(דירה|דירת|חדר|שותפ)|'
        r'צריכ[האים]+\s+(דירה|דירת|חדר)|'
        r'מחפש[תים]?\s+[0-9]'
    )
    try:
        with _conn() as c:
...
def search_listings(filters: Dict, limit: int = 200) -> List[Dict]:
    ...
    _SEEKING_RE = (
        # Russian — "ищем/ищу + [number] + квартиру/комнату"
        r'ищ[уёемаю]\s+(квартир|жиль|комнат|студи|[0-9])|'
        r'ищем\s+(квартир|комнат|жиль|аренд|[0-9])|'
        r'ищем.*комнатн|'
        r'нужна\s+(квартир|комнат)|нужно\s+жиль|'
        ...
        r'срочно\s+ищ[уем]|'
        r'мне\s+нужна\s+квартир|нам\s+нужна\s+(квартир|комнат)|'
        r'в\s+поиске\s+жиль|мы\s+ищем\s+(квартир|комнат|[0-9])|'
        r'сниму\s+(квартир|комнат)|снимем\s+квартир|'
        r'ищу\s+соседк|ищу\s+соседа|ищу\s+сожител|'
        # English
        ...
        r'need\s+(an?\s+)?(apart|flat|room|place\s+to)|'
        # Hebrew — דירת is construct form of דירה, must cover both
        r'מחפש[תים]?\s+(דירה|דירת|חדר|שכירות)|'
        r'מחפשים\s+(דירה|דירת|חדר|להשכיר)|'
        r'מחפשת\s+(דירה|דירת|חדר)|'
        r'בחיפוש\s+(אחר\s+)?(דירה|דירת|חדר)|'
        r'צריכ[האים]+\s+(דירה|דירת|חדר)|'
        r'מחפש[תים]?\s+[0-9]'
    )
